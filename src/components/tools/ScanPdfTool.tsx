...
import { Camera, ImagePlus, ScanLine, ArrowLeft, RefreshCw, Trash2, ChevronLeft, ChevronRight, FileDown, Loader2, AlertCircle, CheckCircle2, X, Sparkles } from 'lucide-react';
...
export default function ScanPdfTool() {
  ...
  const captureFrame = useCallback((): Promise<Blob | null> => new Promise(resolve => {
    ...
    // Flip the preview back when using the front camera, so captures match what
  ...
  const handleCaptureClick = useCallback(async () => {
    ...
    if (!blob) { setError('Could not read the camera frame. Try again.'); return; }
    addPhoto(blob);
    setJustScan(true);
    window.setTimeout(() => setJustScan(false), 350);
        ...
        <div className="bg-white rounded-2xl border border-border p-4 sm:p-6 shadow-sm">
          ...
          {cameraOn ? (
            <div className="relative rounded-2xl overflow-hidden bg-black aspect-[4/3] w-full">
              ...
              <div className="absolute bottom-3 inset-x-0 flex items-center justify-center gap-4">
                <button onClick={handleCaptureClick} className="w-16 h-16 rounded-full bg-white text-primary shadow-lg hover:scale-105 active:scale-95 transition-transform flex items-center justify-center" title="Capture page" aria-label="Capture page">
            ...
            >
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Use your camera to capture pages, or add photos from your device</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  <button onClick={handleOpenCamera} className="flex-1 sm:flex-none px-6 py-3 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 text-white hover:shadow-lg active:scale-[0.98]" style={{ backgroundColor: primaryColor }}>
                    <Camera className="w-4 h-4" /> Open camera
                  </button>
                  <button onClick={() => fileInputRef.current?.click()} className="flex-1 sm:flex-none px-6 py-3 rounded-xl font-medium text-sm border border-border text-foreground hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
              ...
              {fallbackMode && <p className="mt-3 text-xs text-muted-foreground">{fallbackMode}</p>}
            </div>
          )}
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  {cameraOn && (
                    <button onClick={handleCaptureClick} className="px-3 py-1.5 rounded-lg text-xs font-medium text-white transition-colors flex items-center gap-1.5" style={{ backgroundColor: primaryColor }}>
          ...
          {cameraOn && (
            <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
              <span>Tap the shutter to capture a page · tap a thumbnail to reorder or remove</span>
            </div>
          )}
