...
export default function RemovePagesPage() {
  ...
  const faqs = getToolFaqs('remove-pages');

  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateSoftwareApplicationSchema({ name: tool!.name, description: tool!.description, slug: 'remove-pages', category: tool!.category })) }} />
      {faqs.length > 0 && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateFAQSchema(faqs)) }} />}
      <RemovePagesTool />
      <ToolPageSEO slug="remove-pages" />
    </>
  );
}
